...
export const processSolution = async (
  ...
  if (impact.population === 0 && impact.gold === 0 && impact.happiness === 0 && impact.foodSurplus === 0) {
...
export const applyDecisionImpact = (state: NationState, decision: Decision): NationState => {
  ...
  newState.happiness = Math.max(-100, Math.min(100, newState.happiness + decision.impact.happiness));
